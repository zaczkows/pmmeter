...
void BTHome::begin(std::string dname, bool encryption, uint8_t const *const key, bool trigger_based_device) {
    BLEDevice::init("");
...
void BTHome::begin(std::string dname, bool encryption, std::string key, bool trigger_based_device) {
    uint8_t bind_key[BIND_KEY_LEN];
    for (uint8_t i = 0; i < BIND_KEY_LEN; i++) {
        bind_key[i] = strtol(key.substr(i * 2, i * 2 + 2).c_str(), NULL, BIND_KEY_LEN);
    }
    begin(dname, encryption, bind_key, trigger_based_device);
}

void BTHome::setDeviceName(std::string dname) {
    if (!dname.empty())
        this->dev_name = dname;
}
...
void BTHome::buildPacket() {
    ...
    /*
      ...
      bit)+1(COMPLETE_NAME)+1(SERVICE_DATA)+2(UUID)+1(ENCRYPT)+4(nonce)+4(mic)+1(serviceData length bit) if (dn_length >
      BLE_ADVERT_MAX_LEN - this->m_sensorDataIdx - 18) dn_length = BLE_ADVERT_MAX_LEN - this->m_sensorDataIdx - 18;
      }
      else {
        //10=3(FLAG)+1(device name length bit)+1(COMPLETE_NAME)+1(SERVICE_DATA)+2(UUID)+1(ENCRYPT)+1(serviceData length
      bit) if (dn_length > BLE_ADVERT_MAX_LEN - this->m_sensorDataIdx - 10) dn_length = BLE_ADVERT_MAX_LEN -
      this->m_sensorDataIdx - 10;
      }
    ...
    */
    ...
    if (this->m_encryptEnable) {
        ...
        mbedtls_ccm_encrypt_and_tag(&this->m_encryptCTX, this->m_sensorDataIdx, nonce, NONCE_LEN, 0, 0,
                                    &this->m_sensorData[0], &ciphertext[0], encryptionTag, MIC_LEN);
...
void BTHome::start(uint32_t duration) { pAdvertising->start(duration); }

bool BTHome::isAdvertising() { return pAdvertising->isAdvertising(); }
