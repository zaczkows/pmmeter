...
#define MEASUREMENT_MAX_LEN                                                                                            \
    23 // 23=31(BLE_ADVERT_MAX_LEN)-3(FLAG)-1(SERVICE_DATA)-2(UUID)-1(ENCRYPT)-1(serviceData length bit)
...
class BTHome {
  public:
    void begin(std::string dname = "DIY-sensor", bool encryption = false, uint8_t const *const key = NULL,
               bool trigger_based_device = false);
    void begin(std::string dname = "DIY-sensor", bool encryption = false, std::string key = "",
               bool trigger_based_device = false);
